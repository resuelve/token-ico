const ResuelveTokenIco = artifacts.require('./ResuelveTokenIco.sol');
...
contract('ResuelveTokenIco', accounts => {
  ...
  const createToken = () => ResuelveTokenIco.new(fundsWallet, timeController.currentTimestamp(), minCap, maxCap);
  ...
  it('should have initial supply of 1,000,000 units assigned to funds wallet', async () => {
    const resuelveToken = await createToken();
    const expectedSupply = toWei(1000000);

    const totalSupply = await resuelveToken.totalSupply();
    assert.equal(totalSupply, expectedSupply, 'Total supply mismatch');

    const fundsWalletBalance = await resuelveToken.balanceOf(fundsWallet);
  ...
  it('should open at startTimestamp', async () => {
    ...
    const resuelveToken = await ResuelveTokenIco.new(fundsWallet, startTimestamp, minCap, maxCap);

    // should be closed
    await assertExpectedError(resuelveToken.sendTransaction(transaction(buyerOneWallet, oneEth)));

    await timeController.addSeconds(3600);

    // should be open
    await resuelveToken.sendTransaction(transaction(buyerOneWallet, oneEth));
  });

  it('should last 4 weeks if the goal is reached and allow token transfers afterwards', async () => {
    const resuelveToken = await createToken();

    await resuelveToken.sendTransaction(transaction(buyerOneWallet, minCap));
    await timeController.addDays(4 * 7);

    // should be closed
    await assertExpectedError(resuelveToken.sendTransaction(transaction(buyerTwoWallet, oneEth)));

    const totalRaised = await resuelveToken.totalRaised();
    assert.equal(totalRaised.toNumber(), minCap, 'Total raised amount mismatch')

    // REQ004 should allow token transfer
    await resuelveToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet });
  });
  ...
  it('should last more then 4 weeks if the goal is not reached and allow token transfers after closing', async () => {
    ...
    await resuelveToken.sendTransaction(transaction(buyerTwoWallet, minCap - oneEth));

    // should be closed
    await assertExpectedError(resuelveToken.sendTransaction(transaction(buyerThreeWallet, oneEth)));
  ...
  it('should close after the max cap is reached and allow token transfers afterwards', async () => {
    ...
    await resuelveToken.sendTransaction(transaction(buyerTwoWallet, maxCap));
  ...
  it('should transfer raised funds to fundsWallet after each payment', async () => {
    ...
    assert.equal(ethBalance(resuelveToken.address), 0, 'Contract balance mismatch');
    assert.equal(ethBalance(fundsWallet), expectedBalanceGrowth(oneEth), 'Funds wallet balance mismatch');

    await resuelveToken.sendTransaction(transaction(buyerTwoWallet, oneEth));
  ...
  it('should not allow token transfers before ICO', async () => {
    ...
    await assertExpectedError(resuelveToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet }));
  });
  ...
  it('should be sold with +50% bonus during the first week', async () => {
    ...
    const buyerOneBalance = await resuelveToken.balanceOf(buyerOneWallet);
